...
#sampleAnnotationCSV = "/data/scratch/kvalem/projects/2023/pseudomonas_aeruginosa/tables/samplesheet_deseq_batch_effect.csv"
#readCountFile = "/data/scratch/kvalem/projects/2023/pseudomonas_aeruginosa/tables/salmon.merged.gene_counts.tsv"
#c1 = "hAO_inf"
#c2 = "hAO_ctrl"
#batch_col ="batch"
#remove_batch_effect = TRUE
#genes_of_interest = "/data/projects/2023/Pseudomonas_aeruginosa/20_deseq2icbi/04_deseq2_batch_effect/volcano_names/genes_of_interest_volcano.csv"
#organism = "human"
## results_dir = "data/projects/2023/Pseudomonas_aeruginosa/20_deseq2icbi/tmp"
#paired_grp = NULL
#prefix = "nitric_oxide"
# plot_title = NULL
## nfcore=FALSE
...
#cond_col = "group"
#sample_col = "sample"
#contrast = c("group", "hAO_inf", "hAO_ctrl")
#gene_id_type = "ENSEMBL"
#covariate_formula = ""
#fdr_cutoff = 0.1
#fc_cutoff = 0.5
...
sampleAnno <- data.frame(sampleAnno,stringsAsFactors = FALSE)
...
count_mat <- data.frame(count_mat,stringsAsFactors = FALSE)
...
bplapply(names(ddsl), function(dds_name) {
  ...
  #save_plot(file.path(results_dir, paste0(pca_prefix, "_PCA")), p, width=8, height=3)
  ...
  if (remove_batch_effect) {
    ...
    pcaData <- plotPCA(vsd, intgroup=intgroup, returnData=TRUE)
    percentVar <- round(100 * attr(pcaData, "percentVar"))
      group_colors <- c("#9b00ff","#800080", "#008000","#00e300")
      ...
      scale_color_manual(values = group_colors) +  # Set colors for groups
      #scale_color_brewer(type="qual", palette="RdBu") +
      labs(colour="Group" ) +
      labs(shape= "Batch") +
      theme_bw() + 
    ...
    #save_plot(file.path(results_dir, paste0(pca_prefix, "_PCA_after_batch_effect_correction")), p, width=8, height=3)
...
bplapply(names(ora_tests), function(ora_name) {
  ...
  if (!is.null(ora_res)) {
    ...
    ora_res@result  <- subset(ora_res@result, 
                              Description %in% c("response to bacterium",
                                                 "response to lipopolysaccharide",
                                                 "nitric oxide metabolic process","cytokine-mediated signaling pathway","response to interleukin-1"))
    ...
    if (min(res_tab$p.adjust) < 0.05 & length(unique(res_tab$geneID)) > 1) {
      p = dotplot(ora_res, showCategory=5) + scale_y_discrete(labels=function(x) str_wrap(x, width=25))+  theme(axis.text.y  = element_text(size = 20))


      save_plot(file.path(results_dir, paste0(prefix, "_ORA_", ora_name, "_dotplot")), p, width = 7, height = 11)
...
if(!is.null(genes_of_interest)) {
  ...
  lapply(goi_groups, function(grp) {
    ...
    p <- EnhancedVolcano(resIHW,
                    ...
                    #legendLabSize = 7,
                    legendLabels = c('NS', expression(Log[2] ~ FC), expression(italic(adj.p - value)), 
                                     expression(italic(adj.p - value ~ and ~ log[2] ~ FC))),
                    title = plot_title)

    grp_fname = gsub("[^[:alnum:]]+", "_", grp)
    save_plot(file.path(results_dir, paste0(prefix, "_", grp_fname, "_volcano_padj")), p, width = 7, height = 7)
    ...
    grp_fname = gsub("[^[:alnum:]]+", "_", grp)
    save_plot(file.path(results_dir, paste0(prefix, "_", grp_fname, "_volcano_pval")), p, width = 7, height = 7)
